    ...
    public static class BackgroundJobArgsHelper
    {
        public static Type GetJobArgsType(Type jobType)
        {
            foreach (var @interface in jobType.GetInterfaces())
            {
                ...
                if (@interface.GetGenericTypeDefinition() != typeof(IAsyncBackgroundJob<>))
                {
                    continue;
                }
            ...
            throw new FrameworkException($"Could not find type of the job args. " +
                                   $"Ensure that given type implements the  {typeof(IAsyncBackgroundJob<>).AssemblyQualifiedName} interface. " +
